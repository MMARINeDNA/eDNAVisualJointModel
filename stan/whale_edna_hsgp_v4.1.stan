...
//   * kappa, sigma_ct moved from `parameters` to `data`. The qPCR
//     standard-curve calibration is treated as known. sigma_ct is
//     deliberately passed at an inflated value (~0.7) to absorb the
//     Ct ~ log(integer count) discreteness noise that the model can't
//     otherwise account for; see scripts/03_format_stan_data_v4.1.r.
...
model {
  ...
  // kappa, sigma_ct are fixed in the data block (no prior).
