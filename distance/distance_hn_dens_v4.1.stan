...
data {
  ...
  // Upper integration limit for the population-corrected ESW (zero-truncated NB
...
transformed parameters {
  ...
  // Population-corrected ESW: numerical integration of
  // ESW(sigma(s)) over the population group-size distribution. With
  // sigma(s) = exp(log_sigma + beta * (s - s_centre)) convex in s,
  // E[ESW(sigma(s))] > ESW(sigma(E[s])) (population correction); using esw_rep in
...
model {
  ...
  // Encounter rate Poisson per segment, using population-corrected
...
generated quantities {
  ...
  // population group-size distribution (population-corrected).
