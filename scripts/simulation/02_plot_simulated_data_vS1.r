...
grid_coords <- as.matrix(grid_1km[, c("X", "Y")])
...
page1 <- wrap_plots(p_row1, nrow = 1) +
  ...
  plot_annotation(
    title    = "Simulated eDNA fields - vS1 (zero-mean GP)",
...
mb_detect2 <- mb_detect %>% rename(station = mb_sample_idx) %>%
                left_join(.,samples) %>% left_join(.,sp_common)

p_detect <- ggplot(mb_detect2) +
  geom_point(aes(x=X,y=Y,shape=as.factor(detect),color=as.factor(detect)))+
  facet_wrap(~sp_common) +
  scale_shape_manual("Detect",values=c(4,19)) +
  scale_color_manual("Detect",values=c("black","red"))+
  theme_bw()
...
# =============================================================================
# Page 3 - Per-station log(lambda) vs each spatial covariate
# =============================================================================
covariate_levels <- c(
  "X (km, easting)",
  "Y (km, northing)",
...
page2 <- ggplot(page2_df, aes(x = axis_val, y = log_lambda, colour = species)) +
  ...
  labs(
    title    = "Per-station truth vs spatial covariates (vS1)",
...
# =============================================================================
# Page 4 - Water-column sampling-depth effect exp(zsample_pref) per species
# =============================================================================
...
#   plot_annotation(
#     title    = "eDNA water column effect - vS1",
...
# =============================================================================
out_pdf <- "outputs/whale_edna_output_vS1/simulated_edna_fields_vS1.pdf"
