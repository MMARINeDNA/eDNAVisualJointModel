# =============================================================================
# 03_format_stan_data_vS1.R
#
# Read outputs/whale_edna_output_vS1/whale_edna_sim_vS1.rds and assemble
# the Stan data list used by stan/whale_edna_hsgp_vS1.stan. Writes:
#
...
cat(sprintf("  coord_centre: [%.0f, %.0f]\n",#[%.0f, %.0f, %.0f]\n
            coord_centre[1], coord_centre[2]))#, coord_centre[3]))
cat(sprintf("  coord_scale:  [%.0f, %.0f]\n",#[%.0f, %.0f, %.0f]\n
...
# -----------------------------------------------------------------------------
# 4. make set of prediction locations.
# -----------------------------------------------------------------------------

X_km_max     <- sim$meta$X_km_max                  # 500
Y_km_max     <- sim$meta$Y_km_max                  # 1270

# Make and evenly spaced grid across the whole domain

# 1. Define your parameters
N_pred <- 4000   # The defined number of points you want
rect_height <- Y_km_max      # Length of the rectangle (X-axis)
rect_width <- X_km_max        # Width of the rectangle (Y-axis)

# 2. Calculate the optimal number of rows and columns
# We use the aspect ratio to ensure the grid spacing is relatively square
aspect_ratio <- rect_height / rect_width

n_rows <- round(sqrt(N_pred / aspect_ratio))
n_cols <- round(N_pred / n_rows)

actual_points <- n_rows * n_cols 

# 3. Generate the evenly spaced coordinates
# We use length.out to ensure the points span from exactly 0 to the boundaries
x_coords <- seq(0, rect_width, length.out = n_cols)
y_coords <- seq(0, rect_height, length.out = n_rows)

# 4. Combine into a data frame using expand.grid
pred_points <- expand.grid(x = x_coords, y = y_coords)

# 5. Plot the result
# plot(pred_points$x, pred_points$y, 
#      main = paste("Regular Grid:", actual_points, "Points"),
#      ylab = "Height", 
#      xlab = "Width",
#      ylim = c(-1, rect_height + 1), # Added a little buffer to the axis for visibility
#      xlim = c(-1, rect_width + 1),
#      pch = 19,            # Solid circle
#      cex = 1.0,           # Size of the points
#      col = "forestgreen")

# Draw the rectangle border
#rect(0, 0, rect_length, rect_width, border = "black", lwd = 2)

# Convert points to the same transformed scale
N_pred = nrow(pred_points)
pred_coords_norm  <- sweep(pred_points, 2, coord_centre, "-")
pred_coords_norm  <- sweep(pred_coords_norm, 2, coord_scale,  "/")
...
stopifnot(ncol(mb_reads_long) == S+1)

# -- Make indexes for basis functions.
D1 = length(HSGP_M)
if(D1 ==1 ){INDICES <- 1:HSGP_M[1]}
if(D1 ==2 ){INDICES <- expand_grid(1:HSGP_M[1],1:HSGP_M[2])}
if(D1 ==3 ){INDICES <- expand_grid(1:HSGP_M[1],1:HSGP_M[2],1:HSGP_M[3])}
...
stan_data <- list(
  ...
  # Dimensions
  N = as.integer(N),
  N_pred = as.integer(N_pred),
  S = as.integer(S),
  M = as.integer(M_total),
  D1 = as.integer(D1),
  INDICES = INDICES %>% as.matrix() ,

  # Coordinates (normalised to [-1, 1])
  coords      = coords_norm,
  coord_scale = coord_scale,

  pred_coords = pred_coords_norm, # prediction set of coordinates.
  ...
  # Fixed qPCR standard-curve coefficients. vS1: 
  ...
  # gp_l priors 0 in transformed space 
  # with ~30% SD. Other priors unchanged from v4.
  prior_mu_sp_mu         =   2.0,
  prior_mu_sp_sig        =   1.5,
  prior_gp_sigma_shape   =   8.0,
  prior_gp_sigma_rate    =   4.0,
  # These are on the centered and transformed sacle
  prior_gp_raw_alpha   =  c(10), # these are for standardized dimensions (same for x, y ,z)
  prior_gp_raw_beta    =  c(16),  # these are for standardized dimensions (same for x, y ,z)
  
  # These are the metabarcoding observation parameters.
  prior_beta0_phi_mu     =   0.0, prior_beta0_phi_sig    =   1.0,
  prior_gamma0_phi_mu    =   2.0, prior_gamma0_phi_sig   =   1,
