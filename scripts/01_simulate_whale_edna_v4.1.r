# =============================================================================
# simulate_whale_edna_v4.1.R
#
# v4.1 - applies v3.2 lessons to the realistic three-species sim:
# zero-mean GP (so the model's structural assumption matches the truth),
# more sampling stations and depths, and the qPCR-calibration
...
# count noise; see scripts/03_format_stan_data_v4.1.r).
#
# Changes from v4:
#   * **Zero-mean GP** for every species - gauss_pref / zbathy_pref /
#     y_pref are removed. The simulation now matches the Stan model's
#     log(lambda) = mu + f structure exactly, so length-scale and
#     amplitude recovery against the named (lx, ly, lz, sigma) is a
#     clean test rather than the combined preference-plus-residual
#     mismatch that v3 / v4 had.
#   * **500 stations x 6 sample depths** (was 300 x 3). New depths:
#     0, 50, 150, 250, 350, 500 m. Samples with Z_sample > Z_bathy
#     are dropped so N lands somewhere ~1500-2400 in practice.
#   * Junk reads still simulated for realism (matching v4); the Stan
#     model still sees only the 3 target species (junk reads are
#     subtracted before passing to Stan, same as v4).
#
# Domain, bathymetry, station stratification, and the species-specific
# conv_factor / mb-read-depth structure are unchanged from v4.
#
# Two distinct depth processes (unchanged from v3 / v4):
...
# removed in v4.1: with the v3.2 finding that the model treats f_s as
# zero-mean GP, baking deterministic habitat preferences into a
# non-zero GP mean would make length-scale recovery against the named
# (lx, ly, lz) ambiguous (the model would have to absorb the
# preference structure into f_s, mixing scales). With habitat
# preferences gone, the named scales are unambiguously the truth.
...
for (s in seq_len(n_species)) {
  ...
  # Zero-mean GP: f_s ~ MVN(0, K_s). No habitat-preference offset.
...
dir.create("outputs/whale_edna_output_v4.1", showWarnings = FALSE, recursive = TRUE)
saveRDS(sim, "outputs/whale_edna_output_v4.1/whale_edna_sim_v4.1.rds")
