...
#
# Read outputs/whale_edna_output_v4.1/whale_edna_sim_v4.1.rds and assemble
# the Stan data list used by stan/whale_edna_hsgp_v4.1.stan. Writes:
#
#   outputs/whale_edna_output_v4.1/stan_data.rds
#
# Changes from v4 (applying v3.2 lessons):
...
stan_data <- list(
  ...
  # Fixed qPCR standard-curve coefficients. v4.1: kappa, sigma_ct also
  # as data. sigma_ct deliberately set HIGHER than the sim's 0.4 PCR-
  # noise truth: the simulated Ct values also carry discrete-count
  # noise from Ct ~ log(integer aliquot count). For hake at the v4
  # densities (lambda ~ 6, conv_factor 10, vol_filtered 2.5,
  # vol_aliquot/100 = 0.02), expected aliquot copies are ~3 at
  # surface, so Var[log(C)] ~ 1/3 = 0.33, contributing
  # beta_ct * sqrt(0.33) ~ 1.44 * 0.58 = 0.83 of additional Ct SD.
  # Combined with PCR 0.4 gives sqrt(0.4^2 + 0.83^2) ~ 0.92 at surface.
  # At deeper depths (with zsample_pref boost) E[C] is much larger and
  # discrete-count noise is small. Use 0.7 as a per-aliquot average
  # -- still well above the 0.4 truth, but not as extreme as 0.92.
  ...
  # gp_l priors centred on the v4.1 truth (lx, ly, lz) = (50, 300, 150)
