...
p_log_lambda_field_cv <- ggplot(pred_all_loc$log_lambda_pred$Long) +
  geom_tile(aes(x=x,y=y,fill=SD/Mean,color=SD/Mean))+
  scale_fill_viridis_c("CV")+
  scale_color_viridis_c("CV") +
  facet_wrap(~sp_common) +
  theme_bw()

grid.arrange(p_log_lambda_field_mean,p_log_lambda_field_cv)
