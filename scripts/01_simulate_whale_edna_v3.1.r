# =============================================================================
# simulate_whale_edna_v3.1.R  (v3.1)
#
...
#                  cross-shore coordinate (not just X), reflecting the
#                  NNW-SSE trending shelf/slope
#
# Changes from V2:
#   * Extended domain (SF → US/Canada border, ~1270 km N-S).
#   * Bathymetry is a function of a rotated coordinate X_prime = rot(X, Y),
#     so isobaths run NW-SE rather than due N-S.
#   * Species have realistic latitude × bathymetry preferences:
#       - Humpback whale : southern + inshore shelf
#       - PWS dolphin    : northern + offshore slope
#       - Pacific hake   : broadly distributed along the shelf-slope break
#     The (X, Y, Z_bathy) mean function is now log-added back into
#     lambda_true_si (V2 left it out; see comment on that line).
#
# Two distinct depth processes (unchanged from V2):
...
C_obs_si_a_mb[,   n_species+1] <- rep(1:N, each = R)
...
sim <- list(
  ...
  observed = list(
    ...
    mb_reads_rep    = mb_reads_rep
  )
)

dir.create("outputs/whale_edna_output_v3.1", showWarnings = FALSE, recursive = TRUE)
saveRDS(sim, "outputs/whale_edna_output_v3.1/whale_edna_sim_v3.1.rds")
