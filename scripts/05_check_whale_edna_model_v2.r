...
ggsave(file.path(OUTPUT_DIR, "diag_trace.png"),
       mcmc_trace(draws_arr, pars = scalar_pars) + ggtitle("Trace plots"),
       width = 14, height = 10)
...
ggsave(file.path(OUTPUT_DIR, "param_recovery.png"), p_recovery, width = 14, height = 8)
...
p_ls <- ggplot(ls_recovery_df,
  ...
  labs(title = "Length-scale recovery", x = "True length-scale",
       y = "Posterior mean ± 95% CI", colour = "Species", shape = "Dimension") +
  theme_bw(base_size = 11)
ggsave(file.path(OUTPUT_DIR, "lengthscale_recovery.png"), p_ls, width = 9, height = 5)
...
if (length(det_idx) >= 5) {
  ppc_plots[["qpcr_ct"]] <-
    ppc_dens_overlay(y = qpcr_ct_vec[det_idx],
...
for (s in seq_len(S)) {
  ...
  ppc_plots[[paste0("mb_sp", s)]] <-
    ppc_dens_overlay(y = mb_reads_long[, s],
...
p_scatter <- ggplot(plot_df,
  ...
  labs(title = "True vs posterior mean log-λ (surface samples, Z_sample = 0)",
...
ggsave(file.path(OUTPUT_DIR, "lambda_maps.png"),
       wrap_plots(p_maps, nrow = 1), width = 15, height = 5)
...
p_bathy_rec <- ggplot(bathy_recovery_df, aes(x = Z_bathy)) +
  ...
  geom_smooth(aes(y = log(lambda_est + 0.01), colour = species),
              method = "loess", span = 0.4, se = FALSE, linewidth = 1.1) +
  facet_wrap(~species) +
  scale_colour_viridis_d(option = "D", end = 0.85, guide = "none") +
