...
HSGP_M <- c(14L, 6L)#, 32L)    # M_total = 3584
...
if(length(HSGP_M) ==1 ){INDICES <- 1:HSGP_M[1]}
if(length(HSGP_M) ==2 ){INDICES <- expand_grid(1:HSGP_M[1],1:HSGP_M[2])}
if(length(HSGP_M) ==3 ){INDICES <- expand_grid(1:HSGP_M[1],1:HSGP_M[2],1:HSGP_M[3])}
...
stan_data <- list(
  ...
  # gp_l priors centred on the vS1 truth (lx, ly) = (50, 300)
  ...
  prior_gp_raw_lx_mu     =   0.0, prior_gp_raw_lx_sig    =  2.0,
  prior_gp_raw_ly_mu     =   0.0, prior_gp_raw_ly_sig    =  2.0,
  prior_gp_raw_lz_mu     =   0.0, prior_gp_raw_lz_sig    =  2.0,
