...
# All paths in this vignette resolve relative to the .qmd's own
# directory (outputs/distance_v4.1/). Diagnostic PDFs live in this
# folder; the per-species single-page renders for the vignette are in
# `_vignette_figs/`.
```
...
This vignette documents a **stand-alone test pipeline** for the line-transect side, building up from a one-line "fit Distance to simulated data" prototype to a model that:
...
where `λ_s` is the population group density (a single scalar — the model is non-spatial), `L_i` is the segment length, and **`ESW_pop` is the Jensen-corrected ESW averaged over the population group-size distribution**:
$$
...
## PR #36 — Jensen + size-bias corrections + real group sizes + new seed
...
### 3.3.1 Jensen bias fix in the encounter rate

The encounter-rate Poisson previously used `ESW(σ(μ_s))` — the population mean group size plugged into the detection function. But `σ(s) = exp(log σ + β · (s − s_centre))` is convex in `s`, so by Jensen `E[ESW(σ(s))] > ESW(σ(E[s]))`. Using `ESW(σ(μ_s))` *under-estimates* the true mean ESW and inflates `λ_s` to compensate.
...
- `esw_pop` integration uses the rounded-bin log-normal pmf when `group_size_dist == 1`:
  ...
  $$
- Group-size data likelihood: `lognormal_lpdf(s | mu_log, sigma_log)` for PWSD; `neg_binomial_2_lpmf(...)` zero-truncated for humpback. Same Jensen + size-bias corrections.
...
![Humpback whale: simulated density, detection function, prior-vs-posterior of D, and parameter recovery.](_vignette_figs/humpback_page-1.png){width=100%}
...
![PWSD: simulated density, detection function (with group-size covariate), prior-vs-posterior of D, and parameter recovery.](_vignette_figs/pwsd_page-1.png){width=100%}
...
1. **Use Jensen-corrected ESW for the encounter rate.** When detection probability depends on group size and the size distribution has any meaningful spread, `ESW(σ(μ_s))` under-estimates `E[ESW(σ(s))]`. The numerical integration is one extra `for` loop in Stan; the fix takes maybe 30 minutes to write and tests cleanly.
